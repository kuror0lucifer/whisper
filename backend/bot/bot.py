...
def user_id_from_tg_user_id(tg_user_id):
    with conn.cursor() as cursor:
            cursor.execute("SELECT id FROM \"users\" WHERE telegram_id = %s;", (tg_user_id,))
...
def favourite_games_on_sale_button(call: telebot.types.CallbackQuery):
    with conn.cursor() as cursor:
        cursor.execute("SELECT game_id FROM \"favourites\" WHERE user_id = %s AND on_sale = true", (user_id_from_tg_user_id(call.message.chat.id),))
...
def favourite_games_not_on_sale_button(call: telebot.types.CallbackQuery):
    with conn.cursor() as cursor:
        cursor.execute("SELECT game_id FROM \"favourites\" WHERE user_id = %s AND on_sale = false", (user_id_from_tg_user_id(call.message.chat.id),))
        games = cursor.fetchall()
    if len(games) > 0:
        text = '\n'.join([str(game[0]) for game in games])
    else:
...
def favourite_games_all_button(call: telebot.types.CallbackQuery):
    ...
    with conn.cursor() as cursor:
        cursor.execute("SELECT game_id FROM \"favourites\" WHERE user_id = %s", (user_id_from_tg_user_id(call.message.chat.id),))
        games = cursor.fetchall()
    if len(games) > 0:
        text = '\n'.join([str(game[0]) for game in games])
    else:
...
def start(message):
    if len(message.text.split()) > 1 and message.text.split()[0] == '/start':
        ...
        with conn.cursor() as cursor:
            cursor.execute("UPDATE \"users\" SET telegram_id = %s WHERE id = %s;", (telegram_id, token))
    else: 
        pass
...
def force_reminder():
    conn.autocommit = True
    with conn.cursor() as cursor:
        cursor.execute("SELECT game_id FROM \"favourites\" WHERE on_sale = true AND got_reminder = false GROUP BY game_id;")
        games = cursor.fetchall()
    for game in games:
        with conn.cursor() as cursor:
            cursor.execute("SELECT user_id FROM \"favourites\" WHERE game_id = %s", (game[0],))
            users = cursor.fetchall()
        for user in users:
            with conn.cursor() as cursor:
                cursor.execute("SELECT got_reminder FROM \"favourites\" WHERE user_id = %s AND game_id = %s", (user[0], game[0]))
                gotreminder = cursor.fetchone()[0]
                if not gotreminder:
                    cursor.execute("UPDATE \"favourites\" SET got_reminder = true WHERE user_id = %s AND game_id = %s", (user[0], game[0]))
...
def check_sale():
    ...
    with conn.cursor() as cursor:
        cursor.execute("SELECT game_id FROM \"favourites\" GROUP BY game_id;")
        games = cursor.fetchall()
    ...
    if response.status_code == 200:
        ...
        for game in games_info:
            ...
            if (game['price']['salePrice'] is None):
                with conn.cursor() as cursor:
                    cursor.execute("UPDATE \"favourites\" SET on_sale = false, got_reminder = false WHERE game_id = %s", (game['sku'],))
            else:
                with conn.cursor() as cursor:
                    cursor.execute("UPDATE \"favourites\" SET on_sale = true WHERE game_id = %s", (game['sku'],))
